...
export default function Profile({ wallet, fighter, refreshFighter }) {
  ...
  useEffect(() => {
    if (wallet) {
      getBattles(wallet).then(b => { setBattles(b); setLoading(false); });
    }
  }, [wallet]);

  if (!fighter) return <div className="empty-state"><div className="icon">👤</div><p>Connect wallet to view profile</p></div>;
  if (replayBattle) return <BattleView result={replayBattle} wallet={wallet} onClose={() => setReplayBattle(null)} />;
    ...
    <div>
      ...
      <div className="card" style={{ marginBottom: 16 }}>
        <div style={{ display: 'flex', gap: 20, alignItems: 'flex-start', flexWrap: 'wrap' }}>
          <div>{generateAvatar(fighter.avatar_seed, 80)}</div>
          <div style={{ flex: 1 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
              <span style={{ color: '#fff', fontWeight: 700, fontSize: '1.1rem', fontFamily: 'monospace' }}>{shortenWallet(fighter.wallet)}</span>
            ...
            <div style={{ marginTop: 10 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.7rem', color: '#666', marginBottom: 3 }}>
                ...
                <span>Level {fighter.level < 100 ? fighter.level + 1 : 'MAX'}</span>
              </div>
              <div style={{ background: '#1e1e3a', borderRadius: 4, height: 8, overflow: 'hidden' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
        ...
        <div className="card">
          ...
          {fighter.referralBoost > 0 && (
            <div style={{ marginTop: 8, fontSize: '0.72rem', color: '#6bcb77' }}>🔗 +{fighter.referralBoost}% referral boost applied</div>
          )}
        </div>
        ...
        <div className="card">
          ...
          <div className="equip-grid">
            {slots.map(slot => {
              ...
              return (
                <div key={slot} className={`equip-slot ${item ? `rarity-border-${item.rarity}` : 'empty'}`} style={item ? { borderColor: RARITY_COLORS[item.rarity] + '44' } : {}}>
      ...
      <div className="card">
        ...
        {!loading && battles.map((b, i) => {
          const isWinner = b.winner === wallet;
          const opponent = b.challenger === wallet ? b.defender : b.challenger;
          return (
            <div key={b.id || i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0', borderBottom: i < battles.length - 1 ? '1px solid #1a1a2e' : 'none' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <span style={{ fontSize: '0.8rem', color: '#aaa', fontFamily: 'monospace' }}>vs {shortenWallet(opponent)}</span>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <span style={{ fontSize: '0.72rem', color: isWinner ? '#6bcb77' : '#ff6b6b' }}>
                  {isWinner ? `+${b.elo_winner} ELO` : `${b.elo_loser} ELO`}
                </span>
                <button className="btn btn-sm" style={{ background: '#1e1e3a', color: '#aaa', border: '1px solid #2a2a5a', padding: '4px 10px' }} onClick={() => setReplayBattle(b)}>
                  Replay
                </button>
              </div>
            </div>
